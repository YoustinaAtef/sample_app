...
describe UsersController do
  ...
  describe "GET show" do
    ...
    it "should show the user's microposts" do
      ...
      get :show, :id => @user
      response.should have_selector("span", :class => "content" , :content => mp1.content)
      response.should have_selector("span", :class => "content" , :content => mp2.content)
    end
    ...
    it "should display microposts count" do
      ...
      mp2 = Factory(:micropost, :user => @user, :content => "Baz quux")
      get :show, :id => @user
      response.should have_selector("td", :class => "sidebar round", :content => "#{@user.microposts.count}")
    end
    
    describe "when signed in as another user" do
      it "should be successful" do
        test_sign_in(Factory(:user, :email => "another@yahoo.com"))
        get :show, :id => @user
        response.should be_success
      end
    end
  ...
  describe "POST 'create'" do
    ...
    describe "success" do
      ...
      before(:each) do
        @attr ={:name => "new user", :email => "new.user@example.com", :password => "foobar", :password_confirmation => "foobar"}
      end
  ...
  describe "PUT 'update'" do
    ...
    describe "success" do
      ...
      it "should change the user's attributes" do
        put :update, :id => @user, :user => @attr # this changes the attributes of the factory user (@user)
        user = assigns(:user) #get the user in the update action in controller
        @user.reload #reload from the test DB 
        @user.name.should eq(user.name)
        @user.email.should eq(user.email)
        @user.encrypted_password.should eq(user.encrypted_password)
      end
      
      it "should havea flash message" do
        put :update, :id => @user, :user => @attr
  ...
  describe "Authentication of edit/update actions" do
    ...
    describe "for signed-in users" do
      ...
      it "should require a matching users for 'update'" do
        put :update, :id => @user, :user => {}
        response.should redirect_to(root_path)
      end
  ...
  describe "DELETE 'destroy'" do
    ...
    describe "as an admin" do
      ...
      before(:each) do
        @admin = Factory(:user, :email => "hello@yahoo.com", :admin => true)
        test_sign_in(@admin)
      end
      ...
      it "shoudl not be able to destroy itself" do
        lambda do
